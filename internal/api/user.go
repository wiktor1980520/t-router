...
func ChangePasswordHandler(c *gin.Context) {
	...
	var user models.User
	if err := config.DB.Select("id, password_hash").First(&user, "id = ?", userID).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch user"})
		return
	}
	...
	if err := config.DB.Model(&models.User{}).Where("id = ?", userID).Update("password_hash", string(hashedPassword)).Error; err != nil {
